...
// Sets the text font to Lucida Console
#define D2DOV_FONT_LUCIDA			(1 << 8)
...
void DrawLine(float x1, float y1, float x2, float y2, float thickness, D2D1::ColorF color);
...
void DrawBox(float x, float y, float width, float height, float thickness, D2D1::ColorF color, bool filled);
...
void DrawCircle(float x, float y, float radius, float thickness, D2D1::ColorF color, bool filled);

// Allows you to draw an elipse.  Same as a circle, except you have two different radii, for width and height.
void DrawEllipse(float x, float y, float width, float height, float thickness, D2D1::ColorF color, bool filled);

// Draw a string on the screen.  Input is in the form of an std::string.
void DrawString(std::string str, float fontSize, float x, float y, D2D1::ColorF color = D2D1::ColorF(1, 1, 1));
void DrawWString(std::wstring str, float fontSize, float x, float y, D2D1::ColorF color = D2D1::ColorF(1, 1, 1));